...
### 首次配置（在仪表盘「设置」里）
1. **AI API Key**：选择供应商（OpenAI / OpenAI 兼容 / Anthropic），填模型名，粘贴你的 API Key → 「保存 Key」→ 「测试调用」确认连通。Key 会**加密存储**在本地。
2. **自动运行**：勾选「开启每日自动生成」，设定时间（如 09:00）与默认窗口（最近 3 天 / 7 天）→「保存设置」。
3. **配对 token**：复制「浏览器插件配对」里的 token（安装插件时要用）。
4. 「账号池」里导入或添加你关注的商业博主（模板见 `scripts/seed-accounts.example.csv`）。内容必须关联账号池后才会进入正式日报。

> 没有 API Key 也能用：采集、确认、硬筛选、查看候选都正常；只有「生成 AI 日报」这一步需要 Key。
...
导出的文件在 `data/exports/`，数据库在 `data/viral-brief.db`，截图在 `data/screenshots/`。
...
BYOK（自带 Key）。在「设置」里选择：

| 供应商 | provider | 默认 Base URL | 模型名示例 |
|---|---|---|---|
| OpenAI | `openai` | `https://api.openai.com/v1` | `gpt-4o-mini`（分析） / `gpt-4o`（日报） |
| OpenAI 兼容 | `openai-compatible` | 需自填 Base URL | 视服务商而定 |
| Anthropic | `anthropic` | `https://api.anthropic.com` | `claude-haiku-4-5`（分析） / `claude-sonnet-4-6`（日报） |

> OpenAI 兼容接口涵盖大量第三方/代理服务，填好 Base URL 即可。
...
npm test
```
覆盖：指标标准化（文档数据字典全部案例 + 边界）、双 1000 与各数据状态、去重、日报渲染（含数字来自 DB、HTML 防 XSS、CSV BOM）。
